...
%type <gval> program
%type <gval> ClassDeclaration
%type <gval> ClassBody
%type <gval> MembersDeclarationList
%type <gval> MethodsDeclarationList
%type <gval> MethodsDec
...
MethodsDeclarationList: MethodsDec MethodsDeclarationList						{$$.funcList = A_FuncList($1.dec,$2.decList);}
						| MethodsDec											{$$.funcList = A_FuncList($1.dec,NULL);}

MethodsDec:				FUNCTION ID LPAREN RPAREN EQ exp						{$$.dec = A_FunDec(EM_tokPos,S_Symbol($2.sval),NULL,NULL,$6.exp);}
						| FUNCTION ID LPAREN RPAREN COLON ID EQ exp				{$$.dec = A_FunDec(EM_tokPos,S_Symbol($2.sval),NULL,S_Symbol($6.sval),$8.exp);}
						| FUNCTION ID LPAREN TypeFields RPAREN EQ exp			{$$.dec = A_FunDec(EM_tokPos,S_Symbol($2.sval),$4.fieldList,NULL,$7.exp);}
						| FUNCTION ID LPAREN TypeFields RPAREN COLON ID EQ exp	{$$.dec = A_FunDec(EM_tokPos,S_Symbol($2.sval),$4.fieldList,S_Symbol($7.sval),$9.exp);} 
